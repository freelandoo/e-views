...
const log = createLogger("r2.uploadFlameProductImage");
...
module.exports = async function uploadFlameProductImageToR2({ file }) {
  log.info("upload.start", { mimetype: file?.mimetype });
  const fileExt = getFileExt(file.originalname);
  const fileName = `flame-products/${crypto.randomUUID()}.${fileExt}`;
