...
const log = createLogger("FlameService");
...
function walletPayload(wallet, settings, usage) {
  return {
    ...
    limits: {
      ...
      flames_per_ad: settings.flames_per_ad,
      daily_flames_limit: settings.daily_flames_limit,
      cooldown_seconds: settings.cooldown_seconds,
      ads_watched_today: usage.ads || 0,
      flames_earned_today: usage.flames || 0,
...
class FlameService {
  static async getWallet(user) {
    return runWithLogs(log, "getWallet", () => ({ id_user: user?.id_user }), async () => {
      ...
      const [settings, wallet, usage] = await Promise.all([
        FlameStorage.getSettings(pool),
        FlameStorage.getOrCreateWallet(pool, user.id_user),
        FlameStorage.countRewardedToday(pool, user.id_user),
      ]);
  ...
  static async history(user, query) {
    return runWithLogs(log, "history", () => ({ id_user: user?.id_user }), async () => {
      ...
      const transactions = await FlameStorage.listTransactions(pool, user.id_user, { limit, offset });
  ...
  static async requestRewardedAd(user, req) {
    return runWithLogs(log, "requestRewardedAd", () => ({ id_user: user?.id_user }), async () => {
      if (!user?.id_user) return { error: "Não autenticado" };
      const settings = await FlameStorage.getSettings(pool);
      if (!settings?.is_active) return { error: "Sistema de Flames inativo" };
      const usage = await FlameStorage.countRewardedToday(pool, user.id_user);
      if (usage.ads >= settings.ads_per_day_per_user || usage.flames >= settings.daily_flames_limit) {
        return {
          error: "Você atingiu o limite de anúncios de hoje. Volte amanhã para ganhar mais Flames.",
          code: "daily_limit",
        };
      }
      ...
      const reward = Math.min(
        settings.flames_per_ad,
        Math.max(settings.daily_flames_limit - usage.flames, 0)
      );
      if (reward <= 0) return { error: "Limite diário de Flames atingido", code: "daily_limit" };
      const token = crypto.randomUUID();
      const event = await FlameStorage.createRewardEvent(pool, {
      ...
      return {
        ...
        message: `Assista a um anúncio para ganhar ${reward} Flames.`,
      };
    });
  }
  ...
  static async completeRewardedAd(user, body) {
    return runWithLogs(log, "completeRewardedAd", () => ({ id_user: user?.id_user }), async () => {
      ...
      try {
        ...
        const event = await FlameStorage.getRewardEventByToken(client, token);
    ...
    return runWithLogs(log, "spend", () => ({ id_user: user?.id_user, product: body?.product }), async () => {
      ...
      try {
        ...
        if (body.product === "profile_activation") {
          ...
          const sub = await FlameStorage.activateProfileWithFlames(client, {
            user_id: user.id_user,
  ...
  static async getAdminSettings() {
    return { settings: await FlameStorage.getSettings(pool) };
  }
  ...
  static async updateAdminSettings(user, body) {
    ...
    return { settings: await FlameStorage.updateSettings(pool, patch) };
  }

  static async metrics() {
    const metrics = await FlameStorage.metrics(pool);
    ...
    return {
      metrics: {
        ...
        estimated_reward_cost: Number(metrics.flames_issued_today) || 0,
      },
    };
  }
}

module.exports = FlameService;
