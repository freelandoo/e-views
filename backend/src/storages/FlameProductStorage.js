class FlameProductStorage {
  ...
  static async listProducts(conn, { onlyActive = false } = {}) {
    ...
    const { rows } = await conn.query(
      `SELECT *
         FROM public.flame_products
         ${where}
         ORDER BY sort_order ASC, flames_amount ASC, name ASC`
    );
    return rows;
  }
  ...
  static async getProductById(conn, id) {
    const { rows } = await conn.query(
      `SELECT * FROM public.flame_products WHERE id = $1 LIMIT 1`,
      [id]
    );
    return rows[0] || null;
  }
  ...
  static async createProduct(conn, data) {
    ...
    const { rows } = await conn.query(
      `INSERT INTO public.flame_products
         (name, description, image_url, price_cents, flames_amount, bonus_flames, is_active, sort_order)
  ...
  static async updateProduct(conn, id, patch) {
    ...
    values.push(id);
    const { rows } = await conn.query(
      `UPDATE public.flame_products SET ${fields.join(", ")}
       WHERE id = $${i}
       RETURNING *`,
      values
    );
  ...
  static async deleteProduct(conn, id) {
    // Soft-delete via is_active=false (referenciado por flame_purchases).
    const { rows } = await conn.query(
      `UPDATE public.flame_products
          SET is_active = FALSE,
              updated_at = NOW()
        WHERE id = $1
        RETURNING *`,
  ...
  static async createPurchase(conn, data) {
    const { rows } = await conn.query(
      `INSERT INTO public.flame_purchases
  ...
  static async getPurchaseByStripeSession(conn, sessionId) {
    const { rows } = await conn.query(
      `SELECT * FROM public.flame_purchases WHERE stripe_session_id = $1 LIMIT 1`,
      [sessionId]
    );
    return rows[0] || null;
  }
  ...
  static async getPurchaseByPaymentIntent(conn, paymentIntentId) {
    ...
    const { rows } = await conn.query(
      `SELECT * FROM public.flame_purchases WHERE stripe_payment_intent = $1 LIMIT 1`,
      [paymentIntentId]
    );
    return rows[0] || null;
  }

  static async markPurchasePaid(conn, id, { flames_credited, stripe_payment_intent }) {
    const { rows } = await conn.query(
      `UPDATE public.flame_purchases
        ...
        WHERE id = $1
        RETURNING *`,
      [id, flames_credited, stripe_payment_intent || null]
    );
    return rows[0] || null;
  }
  ...
  static async listPurchasesForUser(conn, userId, { limit = 30, offset = 0 } = {}) {
    const { rows } = await conn.query(
      `SELECT pp.*, p.name AS product_name, p.image_url AS product_image_url
         FROM public.flame_purchases pp
         JOIN public.flame_products p ON p.id = pp.product_id
        WHERE pp.user_id = $1
...
module.exports = FlameProductStorage;
