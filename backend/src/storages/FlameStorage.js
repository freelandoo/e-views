class FlameStorage {
  static async getSettings(conn) {
    const { rows } = await conn.query(
      `SELECT * FROM public.flame_settings WHERE id = 1 LIMIT 1`
    );
    return rows[0] || null;
  }
  ...
  static async updateSettings(conn, patch) {
    ...
    const { rows } = await conn.query(
      `UPDATE public.flame_settings SET ${fields.join(", ")} WHERE id = 1 RETURNING *`,
      values
    );
    return rows[0];
  }
  ...
  static async getOrCreateWallet(conn, user_id) {
    const { rows } = await conn.query(
      `INSERT INTO public.flame_wallets (user_id)
       VALUES ($1)
  ...
  static async getWallet(conn, user_id) {
    const { rows } = await conn.query(
      `SELECT * FROM public.flame_wallets WHERE user_id = $1 LIMIT 1`,
      [user_id]
    );
    return rows[0] || null;
  }
  ...
  static async listTransactions(conn, user_id, { limit = 30, offset = 0 } = {}) {
    const { rows } = await conn.query(
      `SELECT *
         FROM public.flame_transactions
        WHERE user_id = $1
  ...
  static async credit(conn, { user_id, wallet_id, amount, type, source, source_id, metadata }) {
    ...
    const { rows: txRows } = await conn.query(
      `INSERT INTO public.flame_transactions
  ...
  static async countRewardedToday(conn, user_id) {
    const { rows } = await conn.query(
      `SELECT COUNT(*)::int AS ads,
              COALESCE(SUM(reward_amount), 0)::int AS flames,
    ...
    return rows[0] || { ads: 0, flames: 0, last_event_at: null };
  }
  ...
  static async metrics(conn) {
    const { rows } = await conn.query(
      `WITH today_tx AS (
       ...
       SELECT
         COALESCE(SUM(CASE WHEN amount > 0 THEN amount ELSE 0 END), 0)::int AS flames_issued_today,
         ABS(COALESCE(SUM(CASE WHEN amount < 0 THEN amount ELSE 0 END), 0))::int AS flames_spent_today,
  ...
  static async activateProfileWithFlames(conn, { user_id, id_profile, amount }) {
    ...
    const { rows } = await conn.query(
      `INSERT INTO public.tb_profile_subscription
       ...
       RETURNING *`,
      [user_id, id_profile, { source: "flames", amount }]
    );
    return rows[0];
  }
}

module.exports = FlameStorage;
