...
import type { FlameTransaction } from "./types"

function token() {
  return typeof window !== "undefined" ? localStorage.getItem("token") : null
}

export function FlamesHistoryModal({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  const [items, setItems] = useState<FlameTransaction[]>([])
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      setLoading(true)
      fetch("/api/flames/history?limit=50", { headers: { Authorization: `Bearer ${t}` } })
        .then((r) => r.json())
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-white/10 bg-zinc-950 text-white sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Histórico de Flames</DialogTitle>
        </DialogHeader>
        {loading ? (
