...
const REWARD_FIELDS: NumericField[] = [
  ...
  {
    key: "daily_flames_limit",
    label: "Limite diario de Flames",
...
const PRICE_FIELDS: NumericField[] = [
  {
    ...
    label: "Ativar perfil",
    helper: "Preco para ativacao de perfil profissional via Flames.",
    suffix: "P",
  },
  {
...
function normalizeSettings(settings: Partial<FlameSettings>): Record<string, string | boolean | number | null> {
...
export function AdminFlamesSettings({ onSaved }: { onSaved: () => void }) {
  ...
  async function load() {
    ...
    setError("")
    try {
      const res = await fetch("/api/admin/flames/settings", {
  ...
  if (loading) {
    return (
      <div className="py-10 text-center text-sm text-muted-foreground">
        ...
        Carregando configuracoes de Flames
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Liga ou desliga ganhos por anuncios e gastos com Flames.
            </p>
          </div>
          <Checkbox
      ...
      <SettingsSection
        title="Precos em Flames"
        description="Todos os precos editaveis da tabela flame_settings. Manifestacao usa preco por produto no catalogo."
      >
        <div className="grid gap-4 sm:grid-cols-2">
      ...
      <SettingsSection title="Metadados" description="Campos readonly do registro singleton de flame_settings.">
