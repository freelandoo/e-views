...
/**
 * Versão fina do FlamesCard — colada como footer do headcard.
 * Mostra saldo + progresso diário compacto + 3 ações pequenas
 * (loja, assistir anúncio, histórico).
 */
export function HeadcardFlamesFooter({ className }: { className?: string }) {
  ...
  const load = useCallback(async () => {
    ...
    if (!t) return
    try {
      const res = await fetch("/api/flames/wallet", {
  ...
  const dailyEarned = limits?.flames_earned_today ?? 0
  const dailyMax = limits?.daily_flames_limit ?? 0
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="leading-tight">
            <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-amber-200/70">Meus Flames</div>
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            title={reachedAdLimit ? "Limite diário atingido" : "Assistir anúncio para ganhar Flames"}
          >
...
export default HeadcardFlamesFooter
